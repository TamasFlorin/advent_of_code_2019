...
def execute_program(program, inputs=[]):
    ...
    while current_index < len(program):
        ...
        if opcode == 99:
            ...
            return current_outputs[len(current_outputs) - 1]
            ...
            if opcode == 1 or opcode == 2:
                ...
                current_outputs.append(int(get_value_from_mode(address, modes[2], program)))
                current_index += 2
            elif opcode == 5:
...
def thrust(program, inpt, thrusters, steps=0, used=set()):
    if steps == thrusters:
        return inpt
    
    values = []
    for i in range(thrusters):
        used_new = copy.deepcopy(used)
        if i not in used:
            used_new.add(i)
            setting = execute_program(copy.deepcopy(program), inputs=[i, inpt])
            values.append(thrust(copy.deepcopy(program), setting, thrusters, steps + 1, used=used_new))
    return max(values)

def solve_rec(program, thrusters=5):
    return thrust(program, 0, thrusters)

if __name__ == "__main__":
    program = read_program(sys.argv[1])
    print(solve_rec(program))